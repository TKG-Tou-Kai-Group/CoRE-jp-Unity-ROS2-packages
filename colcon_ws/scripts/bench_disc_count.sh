#!/bin/bash
# physics_hz とロボット台数は据え置きで、1 台あたりのディスク枚数だけを振る。
#
#   ./scripts/bench_disc_count.sh [physics_hz] [robots]
#
# ロボット 2 台で 200 Hz が実時間に収まらなくなる原因が、積んである
# ディスク (接触したまま積み重なった剛体) なのかを確かめる。
#
# 物理の負荷だけを見たいので ros2_control は起動せず、spawn_entity と
# spawn_flying_discs でシミュレータに実体を置くだけにする。カメラは
# シミュレータ側のセンサなので、コントローラが無くても publish される。

HZ=${1:-200}
ROBOTS=${2:-2}
SIM_DIR=${SIM_DIR:-$HOME/Unity_ROS2_Robot_Simulator_v1.0.5_Linux_amd64}
WS=${WS:-$HOME/colcon_ws}
CFG=/tmp/sim_resources_disccount_${HZ}.json
LOG=/tmp/disccount_${HZ}

source /opt/ros/jazzy/setup.bash
source "${WS}/install/setup.bash"

# spawn 位置は sample_robot_N_spawn_for_core.launch.py と同じ
POS_X=(-4.5  4.5 -5.5)
...
# URDF を用意する (xacro 展開は launch と同じもの)
python3 - <<'PY'
import os
import xacro
...
echo "############ ディスク枚数とフレームレート (physics_hz=${HZ}, ロボット${ROBOTS}台) ############"

for COUNT in 0 5 10 20; do
  start_sim || exit 1
  ...
  for n in $(seq 1 "${ROBOTS}"); do
    ...
    if [ "${COUNT}" -gt 0 ]; then
      ros2 run core_sim_utils spawn_flying_discs --ros-args \
        -p urdf_path:=/tmp/bench_disc.urdf \
        -p name_prefix:=sample_robot_${n}_flying_disc \
        -p count:="${COUNT}" \
        -p x:="${POS_X[$i]}" -p y:="${POS_Y[$i]}" -p z:=0.70 \
        -p R:=0.0 -p P:=0.0 -p Y:="${POS_YAW[$i]}" \
        -p z_spacing:=0.021 > /dev/null 2>&1
    fi
  done
  sleep 30   # 積み荷が落ち着くまで

  echo
  echo "===== [${HZ}] ロボット${ROBOTS}台 / 1台あたりディスク ${COUNT} 枚 (計 $((COUNT * ROBOTS)) 枚) ====="

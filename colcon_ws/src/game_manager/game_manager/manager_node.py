...
class GameManager(Node):
    ...
    def __init__(self):
        ...
        # Isaac 版はシミュレータ側のプロセスと共有メモリ ('isaac_sim_reset') で
        # リセットを伝えていたが、Unity 版はシミュレータが別プロセスなので
        # simulation_interfaces の reset_simulation サービスを使う。
        # SCOPE_STATE は「スポーンしたエンティティの姿勢・速度を spawn 時点へ戻す」で、
        # ロボットもディスクも初期配置に戻る。SCOPE_SPAWNED (デスポーン) は使わない。
    ...
    def request_sim_reset(self):
        """シミュレータへリセットを要求する (非同期)。

        タイマーコールバックから呼ばれるので、応答待ちでブロックしない。
        シミュレータが繋がっていなくても試合進行は止めない。
        """
        if not self.reset_client.service_is_ready():
            self.get_logger().warning(
                'reset_simulation service is not available; skipping simulator reset')
            return
        if self.reset_future is not None and not self.reset_future.done():
            self.get_logger().warning('previous reset_simulation call is still pending')
            return
        request = ResetSimulation.Request()
        request.scope = ResetSimulation.Request.SCOPE_STATE
        self.reset_future = self.reset_client.call_async(request)
        self.get_logger().info('reset_simulation (SCOPE_STATE) requested')

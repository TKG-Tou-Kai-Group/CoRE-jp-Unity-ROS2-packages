...
# 28 ストロークで射出 0 発だった)。
#
# z は通路の中に直接置く。シュータは base_link の +0.45 にあり、底板の上面が
# +0.490、ディスクは厚み 0.02 なので中心を +0.500 に置くと底板に載って
# 射出輪の帯 (+0.490〜+0.520) に入る。
#
# 上から落とす方式は採れない。バレル (押さえ) の下面が +0.515 にあるので、
# それより高い位置から落とすと通路に入らず天井の上に乗ってしまう
# (以前 20 枚積み方式の +0.55 をそのまま流用していて、飛翔 0 発になった)。
CHUTE_X = 0.0
CHUTE_Z = 0.50

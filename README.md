...
> **注意**: 既定の `physics_hz` は 200 です。この設定では**初速が 16.4〜19.6 m/s と
> 発射ごとに振れます** (800 Hz では 19.7〜20.0 に収まる)。ディスクが射出輪に噛む
> 区間はごく短いので、刻みが粗いと「何ステップ噛んだか」が発射ごとに変わるためです。
> 射出の再現性が要る用途では
> [config/simulation_resources.json](colcon_ws/src/sample_robot_sim/config/simulation_resources.json)
> の `physics_hz` を上げてください。

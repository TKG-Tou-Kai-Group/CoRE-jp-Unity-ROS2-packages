...
>
> - **v1.2.1**: IMU の角速度の符号と時間刻みが誤っています (v1.2.2 で修正)。
>   IMU を使わないなら支障ありません
>
...
ディスクは装填機構 (`shooter_loader_link_joint`) が通路の中を押し出し、
逆回転する 2 枚の射出輪に挟まれて加速します。ros2_control が担うのはこの
2 つだけで、走行は上のとおりシミュレータ側です。

弾は `core_sim_utils/flying_disc_feeder` が **1 ストロークにつき 1 枚**供給します。
装填機構が押し出しから引き戻しへ転じ、**引き切ったのを確認してから** 1 枚置きます。
引き切る前に置くと、ローダーの実体と重なった位置にディスクが出現し、貫入を
解消する力積で後ろへ吹き飛びます。

### 実測の弾道特性

`physics_hz` 800、16 発 × 6 回 (96 発) での実測です。

| 項目 | 値 |
|---|---|
| 装填 | 96 枚 / 96 ストローク (取りこぼし・二重装填なし) |
| 後ろへ飛んだ弾 | 0 発 |
| 初速 | 中央 20.2 m/s (回ごとの中央が 19.9〜20.3、理論値 20) |
| 仰角 | 中央 +3.0 度、1 回あたりの幅 3.9 度 |
| **方位** | **中央 0 度付近、1 回あたりの幅 11.4 度 (四分位で 6.8 度)** |

**方位は ±6 度ほど散ります。**4 m 先で ±0.42 m に相当し、装甲板に当たり得る
横ずれの許容 (0.165 m) を超えるので、狙っても外れる弾が出ます。偏りは無く
(中央はほぼ 0 度)、全機体が同じ機構なので条件は公平です。

このばらつきは**剛体 2 つの接触で加速する方式に由来する**もので、寸法や物理
パラメータでは動きませんでした。次の 6 条件をそれぞれ 6 回ずつ測りましたが、
いずれも測定のばらつき (四分位 4.5〜6.8 度、幅 10.5〜18.3 度) の帯から出ません。

- 通路の隙間 `wall_clearance` 1.5 / 0.75 / 0.3 mm
- 通路の壁を射出輪の引き継ぎ位置 (x = 0.1336) まで延長
- 食い込み `roller_squeeze` 2 → 1 mm
- 物理レート 200 / 400 / 800 Hz
- ソルバ反復 既定 / 16

**同じ調整を再度試す価値はありません。**消したい場合は、摩擦輪で加速するのを
やめてシミュレータ側から直接速度を与える方式が要ります。

> **注意**: 既定の `physics_hz` は 200 です。この設定では**初速が発射ごとに
> 16.4〜19.6 m/s と振れます** (理論値 20)。ディスクが射出輪に噛む区間はごく短いので、
> 刻みが粗いと「何ステップ噛んだか」が発射ごとに変わるためです。
>
> **8 台構成では、これを直せる `physics_hz` はありません。**
>
> | `physics_hz` | 8 台の RTF | 8 台のカメラ (実質) | 射出の初速 (1 台) |
> |---|---|---|---|
> | **200 (既定)** | 1.03〜1.04 | 10.31 Hz | 16.4〜19.6 m/s |
> | 400 | 1.02〜1.04 | 10.44 Hz | 17.6〜20.9 m/s (方位も悪化) |
> | 800 | 1.02〜1.04 | **1.35 Hz** | 19.9〜20.3 m/s |
>
> 800 Hz なら初速は揃いますが、**8 台ではカメラが崩壊します**。1 フレームあたりの
> 物理ステップが 13 回になり描画が追いつかず、描画レートが 5 FPS まで落ちて
> カメラの周期がそれに引きずられるためです。**RTF は 1.03 のままなので、RTF だけを
> 見ていると気づけません。**機体が 1〜2 台なら 800 Hz は使えます。
